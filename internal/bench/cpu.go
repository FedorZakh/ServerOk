...
func measure(ctx context.Context, w workload, n int, secs float64) float64 {
	...
	// Секундомер останавливается ДО выставления флага и ожидания горутин:
	// время, за которое рабочие горутины заметят stop, — не время измерения,
	// и его включение занижало бы результат.
